  ...
  const handleAnswer = (selectedValue: string) => {
    ...
    if (nextQuestionIndex < questions.length) {
      ...
      setTimeout(() => {
         ...
         // You should call the onComplete prop here, passing the answers
         // This prop will be handled by the parent component (QuestionnairePage)
         // For now, let's add a placeholder call to onComplete
         // onComplete(answers); // Uncomment this line when you add the prop to QuestionnairePage
      }, 300); // Short delay for animation
    }
  };
  ...
  if (isFinished) {
    return (
      ...
      >
        ...
        <p className="text-lg">You've completed the questionnaire. We'll use your answers to personalize your experience.</p>
        {/* You could add a button here to proceed to the next part of the app */}
        {/* For now, the QuestionnairePage will handle the navigation via the onComplete prop */}
      </motion.div>
    );
  }
